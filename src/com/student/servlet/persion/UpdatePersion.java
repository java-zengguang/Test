...
public class UpdatePersion extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdatePersion() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//PersionManagerInte smi=(PersionManagerInte) Util.getBiz("PersionManagerBiz",request);
		Session ss=Util.getSession(request);
		//PersionManagerInte smi=(PersionManagerInte) ss.getBiz("Tb_persion");
		...
		for(int i=0;i<in.length;i++){
			boolean b=ss.update("Tb_persion",new Tb_persion(new String(name[i].getBytes("ISO-8859-1"),"GBK"),
